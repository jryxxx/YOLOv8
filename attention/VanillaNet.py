...
class VanillaBlock(nn.Module):
    ...
    def switch_to_deploy(self):
        ...
        self.conv.weight.data = torch.matmul(kernel.transpose(
            1, 3), self.conv1[0].weight.data.squeeze(3).squeeze(2)).transpose(1, 3)
        self.conv.bias.data = bias + \
            (self.conv1[0].bias.data.view(1, -1, 1, 1)
             * kernel).sum(3).sum(2).sum(1)
...
class VanillaNet(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, dims=[96, 192, 384, 768],
        ...
        for i in range(len(strides)):
            if not ada_pool:
                ...
                stage = VanillaBlock(dim=dims[i], dim_out=dims[i+1], act_num=act_num,
    ...
    def switch_to_deploy(self):
        ...
        self.stem1[0].bias.data = bias + \
            (self.stem1[0].bias.data.view(1, -1, 1, 1)
             * kernel).sum(3).sum(2).sum(1)
        ...
        self.cls1[2].weight.data = torch.matmul(kernel.transpose(
            1, 3), self.cls1[2].weight.data.squeeze(3).squeeze(2)).transpose(1, 3)
        self.cls1[2].bias.data = bias + \
            (self.cls1[2].bias.data.view(1, -1, 1, 1)
             * kernel).sum(3).sum(2).sum(1)
...
def vanillanet_5(pretrained='', in_22k=False, **kwargs):
    model = VanillaNet(dims=[128*4, 256*4, 512*4, 1024*4],
                       strides=[2, 2, 2], **kwargs)
    if pretrained:
...
def vanillanet_7(pretrained='', in_22k=False, **kwargs):
    model = VanillaNet(dims=[128*4, 128*4, 256*4, 512*4,
                       1024*4, 1024*4], strides=[1, 2, 2, 2, 1], **kwargs)
    if pretrained:
        weights = torch.load(pretrained)['model_ema']
        model.load_state_dict(update_weight(model.state_dict(), weights))
    return model
...
def vanillanet_8(pretrained='', in_22k=False, **kwargs):
    model = VanillaNet(dims=[128*4, 128*4, 256*4, 512*4, 512*4,
                       1024*4, 1024*4], strides=[1, 2, 2, 1, 2, 1], **kwargs)
    if pretrained:
        weights = torch.load(pretrained)['model_ema']
        model.load_state_dict(update_weight(model.state_dict(), weights))
    return model
...
def vanillanet_9(pretrained='', in_22k=False, **kwargs):
    model = VanillaNet(dims=[128*4, 128*4, 256*4, 512*4, 512*4, 512*4,
                       1024*4, 1024*4], strides=[1, 2, 2, 1, 1, 2, 1], **kwargs)
    if pretrained:
        weights = torch.load(pretrained)['model_ema']
        model.load_state_dict(update_weight(model.state_dict(), weights))
    return model
...
def vanillanet_11(pretrained='', in_22k=False, **kwargs):
    model = VanillaNet(
        dims=[128*4, 128*4, 256*4, 512*4, 512*4,
              512*4, 512*4, 512*4, 1024*4, 1024*4],
...
def vanillanet_12(pretrained='', in_22k=False, **kwargs):
    model = VanillaNet(
        dims=[128*4, 128*4, 256*4, 512*4, 512*4, 512 *
              4, 512*4, 512*4, 512*4, 1024*4, 1024*4],
...
def vanillanet_13(pretrained='', in_22k=False, **kwargs):
    model = VanillaNet(
        dims=[128*4, 128*4, 256*4, 512*4, 512*4, 512*4,
              512*4, 512*4, 512*4, 512*4, 1024*4, 1024*4],
...
def vanillanet_13_x1_5(pretrained='', in_22k=False, **kwargs):
    model = VanillaNet(
        dims=[128*6, 128*6, 256*6, 512*6, 512*6, 512*6,
              512*6, 512*6, 512*6, 512*6, 1024*6, 1024*6],
        strides=[1, 2, 2, 1, 1, 1, 1, 1, 1, 2, 1],
        **kwargs)
    if pretrained:
        weights = torch.load(pretrained)['model_ema']
