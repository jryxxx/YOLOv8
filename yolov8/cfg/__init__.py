...
from yolov8.utils import (ASSETS, DEFAULT_CFG, DEFAULT_CFG_DICT, DEFAULT_CFG_PATH, LOGGER, RANK, ROOT, RUNS_DIR,
...
def entrypoint(debug=''):
    ...
    if 'rtdetr' in stem:  # guess architecture
        from yolov8 import RTDETR
        model = RTDETR(model)  # no task argument
    elif 'fastsam' in stem:
        from yolov8 import FastSAM
        model = FastSAM(model)
    elif 'sam' in stem:
        from yolov8 import SAM
        model = SAM(model)
    else:
