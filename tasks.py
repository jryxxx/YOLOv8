...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='ultralyticsn.yaml', ch=3, nc=None, verbose=True):
        """Initialize the ultralytics detection model with the given config and parameters."""
        super().__init__()
...
class SegmentationModel(DetectionModel):
    ...
    def __init__(self, cfg='ultralyticsn-seg.yaml', ch=3, nc=None, verbose=True):
        """Initialize ultralytics segmentation model with given config and parameters."""
...
class PoseModel(DetectionModel):
    ...
    def __init__(self, cfg='ultralyticsn-pose.yaml', ch=3, nc=None, data_kpt_shape=(None, None), verbose=True):
        """Initialize ultralytics Pose model."""
...
class ClassificationModel(BaseModel):
    ...
    def __init__(self, cfg='ultralyticsn-cls.yaml', ch=3, nc=None, verbose=True):
        """Init ClassificationModel with YAML, channels, number of classes, verbose flag."""
        super().__init__()
        self._from_yaml(cfg, ch, nc, verbose)

    def _from_yaml(self, cfg, ch, nc, verbose):
        """Set ultralytics model configurations and define the model architecture."""
        self.yaml = cfg if isinstance(
            cfg, dict) else yaml_model_load(cfg)  # cfg dict
        ...
        if e.name == 'models':
            raise TypeError(
                emojis(f'ERROR ❌️ {weight} appears to be an Ultralytics YOLOv5 model originally trained '
                       ...
                       f'ultralytics at https://github.com/ultralytics/ultralytics.'
                       f"\nRecommend fixes are to train a new model using the latest 'ultralytics' package or to "
                       f"run a command with an official ultralytics model, i.e. 'yolo predict model=ultralyticsn.pt'")) from e
        LOGGER.warning(f"WARNING ⚠️ {weight} appears to require '{e.name}', which is not in ultralytics requirements."
                       f"\nAutoInstall will run now for '{e.name}' but this feature will be removed in the future."
                       f"\nRecommend fixes are to train a new model using the latest 'ultralytics' package or to "
                       f"run a command with an official ultralytics model, i.e. 'yolo predict model=ultralyticsn.pt'")
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        ...
        # TEST 遍历块，加入新模块时，要将模块名加入到下面的列表中
        # TAG 主要作用：重新确定网络中每一层的输入、输出通道大小，对网络中每一层参数重新判别更改，
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3,
                 # TEST 添加新模块
                 C2f_LSKA_Attention, CoordAtt):
            c1, c2 = ch[f], args[0]
            ...
            args = [c1, c2, *args[1:]]
            # TEST 重复模块，新模块加入时，要将模块名加入到下面的列表中
...
def yaml_model_load(path):
    """Load a ultralytics model from a YAML file."""
    import re
    ...
    # i.e. ultralyticsx.yaml -> ultralytics.yaml
